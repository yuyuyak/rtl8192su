	...
	struct net_device * dev)
{
	...
	if(priv->rtllib->iw_mode == IW_MODE_ADHOC || 
		priv->rtllib->iw_mode == IW_MODE_MASTER || priv->rtllib->iw_mode == IW_MODE_APSTA)
#else
	if(priv->rtllib->iw_mode == IW_MODE_ADHOC)
#endif
	{
...
#ifdef ASL
		|| ((priv->rtllib->state == RTLLIB_LINKED) && (priv->rtllib->iw_mode == IW_MODE_APSTA))
#endif
#ifdef _RTL8192_EXT_PATCH_			
...
#ifdef ASL
	if ((priv->rtllib->ap_state == RTLLIB_LINKED) && (priv->rtllib->iw_mode == IW_MODE_MASTER || priv->rtllib->iw_mode == IW_MODE_APSTA)) {
		int	i;
		struct sta_info *pEntry;
